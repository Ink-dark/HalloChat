...
const lengthTestCases = [
  { length: 5, expected: false, description: '5位密码应该失败' },
  { length: 6, expected: true, description: '6位密码应该通过' },
  { length: 10, expected: true, description: '10位密码应该通过' },
  { length: 20, expected: true, description: '20位密码应该通过' },
  { length: 21, expected: false, description: '21位密码应该失败' },
];

lengthTestCases.forEach(testCase => {
  // 创建符合要求的密码
  const password = 'A' + 'a' + '1' + '!'.repeat(Math.max(0, testCase.length - 3));
  const result = USER.PASSWORD.PATTERN.test(password);
  const isValid = result === testCase.expected;
  
  console.log(`${isValid ? '✅' : '❌'} ${testCase.description}: 长度${testCase.length}, 结果${result} (期望${testCase.expected})`);
});
