...
class EncryptedChatService {
  constructor() {
    ...
    this.serverAddress = localStorage.getItem('halloChat_server') || 'http://localhost:7932';
  }
  ...
  connect(user, encryptionKey) {
    ...
    this.socket = io(`${this.serverAddress}/encrypted`, {
      query: { 
        userId: user.id,
        isEncrypted: true
      }
  ...
  setServerAddress(address) {
    ...
    localStorage.setItem('halloChat_server', address);
  }
}
